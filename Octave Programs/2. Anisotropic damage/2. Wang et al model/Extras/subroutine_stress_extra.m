function [sig6,A66,sdvl]=subroutine_stress_extra(eps6,sdvl,ttype)
...
%%%%%%  Terms in damage evolution equations  %%%%%%%%
    if sig6_eff(1) >= 0
      ...
      T_f = sqrt((eps(1)/eps_11_f_t)**2 + (eps(4)/eps_12_f)**2 + (eps(5)/eps_13_f)**2);
      
    elseif sig6_eff(1) < 0
   
      k1 = (-sig_11_f_c*eps_11_f_c*L_c)/G_c_1;
      T_f = sqrt((eps(1)/eps_11_f_c)**2);
    ...
    if sig6_eff(2) >= 0 
      ...
      T_m = sqrt(((eps(2)+eps(3))**2/(eps_22_f_t*eps_33_f_t)) -  (eps(2)*eps(3)/eps_23_f**2) +  (eps(4)/eps_12_f)**2 + (eps(5)/eps_13_f)**2 + (eps(6)/eps_23_f)**2);
      
    elseif sig6_eff(2) < 0
  
      k2 =  (-sig_22_f_c*eps_22_f_c*L_c)/G_c_2;
      T_m = sqrt(((eps(2)+eps(3))**2/(eps_22_f_c*eps_33_f_c)) + ((eps(2)+eps(3)/eps_22_f_c)*( (eps_22_f_c/2*eps_12_f)  - 1))   -  (eps(2)*eps(3)/eps_23_f**2) +  (eps(4)/eps_12_f)**2 + (eps(5)/eps_13_f)**2 + (eps(6)/eps_23_f)**2);
    ...
    if sig6_eff(3) >= 0
      ...
      T_z = sqrt((eps(3)/eps_33_f_t)**2 + (eps(5)/eps_23_f)**2 + (eps(6)/eps_13_f)**2);
      
    elseif sig6_eff(3) < 0

      k3 =  (-sig_33_f_c*eps_33_f_c*L_c)/G_c_3;
      T_z = sqrt((eps(3)/eps_33_f_c)**2 + (eps(5)/eps_23_f)**2 + (eps(6)/eps_13_f)**2);
...
elseif ttype == 1
    ...
    for ieps=1:1:length(eps6)
        ...
        [sig6per,Adummy,sdvldummy] = subroutine_stress_extra(epsper,sdvl,0);
