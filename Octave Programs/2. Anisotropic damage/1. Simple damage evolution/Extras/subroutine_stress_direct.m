function [sig6,A66,sdvl]=subroutine_stress_direct(eps6,sdvl,ttype)
...
%%%%%%  Terms in damage evolution equations  %%%%%%%%
    ...
    M(2,2) = (1-d2);
    M(3,3) = (1-d3);
    M(4,4) = sqrt((1-d1)*(1-d2));
    M(5,5) = sqrt((1-d2)*(1-d3));
    M(6,6) = sqrt((1-d1)*(1-d3));
    M;

    for i = 1:6
       for j = 1:6
          sig6(i) = sig6(i) + M(i,j)*sig6_eff(j);
       end
    end    
...
elseif ttype == 1
    ...
    for ieps=1:1:length(eps6)
        ...
        [sig6per,Adummy,sdvldummy] = subroutine_stress_direct(epsper,sdvl,0);
