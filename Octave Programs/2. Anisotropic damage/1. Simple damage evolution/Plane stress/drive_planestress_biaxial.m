...
s11=zeros(1,steps);
s22=zeros(1,steps); eps12=zeros(1,steps);
...
for n=1:steps
    ...
    while norm(sbar) > tol % check convergence
        ...
        [s,A,sdvup]=subroutine_stress(epsilon,sdv(:,n),ttype);
        
        % 3.) partitioning
        sbar=partition_planestress_Bi(s);
        Abar=partition_planestress_Bi(A);
    ...
    s11(n+1)=s(1);
    s22(n+1)=s(2); eps12(n+1)=epsilon(3); 
...
data = [time; e11; s11; eps12; ];

fileID = fopen('output_planestress_biaxial.txt','w');
fprintf(fileID,'%2s %15s %15s %15s\n','n','Epsilon_{11}','Sigma_{11}','Epsilon_{12}');
fprintf(fileID,'%25s\n','');
